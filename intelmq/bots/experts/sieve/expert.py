...
class SieveExpertBot(ExpertBot):
    ...
    _cond_map: CondMap = {
        ...
        "IpRangeMatch": lambda self, match, event: self.process_ip_range_match(
            match.key, match.range, event
        ),
        "DateMatch": lambda self, match, event: self.process_date_match(
            match.key, match.op, match.date, event
        ),
    ...
    def process_clause(self, clause, event, else_clause=False) -> Optional[Procedure]:
        if else_clause or self.match_expression(clause.expr, event):
            ...
            for procedure in (
                self.process_statement(statement, event)
    ...
    def process_multi_string_match(self, key, op, value, event) -> bool:
        ...
        return self._string_multi_op_map[op](
            event[key], (v.value for v in value.values)
        )
    ...
    def process_multi_numeric_match(self, key, op, value, event) -> bool:
        ...
        return self._numeric_multi_op_map[op](
            event[key], (v.value for v in value.values)
        )
        ...
        if name == "SingleNumericValue":
            ...
            return any(
                self.process_numeric_operator(event[key], op, val.value)
    ...
    def process_list_match(self, key, op, value, event) -> bool:
        ...
        rhs = value.values
        return (
            lhs == rhs if op == ":equals" else self._list_op_map[op](set(lhs), set(rhs))
        )
